...
def test_synthetic_state_spec():
    ...
    # Invalid PN
    spec2 = SyntheticStateSpec(
        trigger="test",
        pn_override=1.5  # Invalid!
    )
...
def test_cli_programmatic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        agent = ConsciousAgent(
            workspace=workspace,
            self_id=f"{tmpdir}/test_identity"
        )
        ...
        result = cmd_handler._cmd_inject_state(
            "test crisis situation --pn=0.7 --crisis"
        )
        print(f"   Command executed successfully: {result}")
        
        # Check if state was integrated
        crisis_count = len([exp for exp in agent.persistent_self.formative_experiences 
        ...
        result = cmd_handler._cmd_inject_state(
            "complex multi word trigger --pn=0.5"
        )
